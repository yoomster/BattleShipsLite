...
namespace BattleShipLiteLibrary
{
    public static class GameLogic
    {
        ...
        public static bool StoreShip(PlayerInfoModel model, string location)
        {
            ...
            bool isValidShipLocation = ValidateGridLocation(model,row,column);
            bool isSpotOpen = ValidateShipLocation(model, row, column);

            if (isValidShipLocation && isSpotOpen)
            {
        ...
        public static bool IdentifyShotResult(PlayerInfoModel opponent, string row, int column)
        {
            ...
            foreach (var ship in opponent.ShipLocations)
            {
                if (ship.SpotLetter == row.ToUpper() && ship.SpotNumber == column)
                {
                    isAHit = true; // NEED TO ADD A CHANGE TO THE SHIP
        ...
        public static void MarkShotResult(PlayerInfoModel player, string row, int column, bool isAHit) //WERKT
        {
            foreach (var gridSpot in player.ShotGrid)
            {
                if (gridSpot.SpotLetter == row.ToUpper() && gridSpot.SpotNumber == column)
